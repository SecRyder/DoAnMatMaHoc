...
def xuLyBruteForce():
    entryStartTime.config(state='normal')
    entryEndTime.config(state='normal')
    entryTime.config(state='normal')
    entryFrequency.config(state='normal')
    entryStartTime.delete(0, END)
    entryEndTime.delete(0, END)
    ...
    pl = entryPlaintext.get()
    d = DES()
    k, t, f, start_time_str, end_time_str = d.bruteforce(cp, pl, textProcess)
    entryKey.delete(0, END)
    entryKey.insert(0, k)
    if start_time_str:
        entryStartTime.insert(0, start_time_str)
    if end_time_str:
        entryEndTime.insert(0, end_time_str)
    if t is not None:
        entryTime.insert(0, t)
    entryFrequency.insert(0, f)
    entryStartTime.config(state='readonly')
    entryEndTime.config(state='readonly')
    entryTime.config(state='readonly')
    entryFrequency.config(state='readonly')
...
def xuLyBruteForceDictionary():
    ...
    pl = entryPlaintext.get()
    d = DES()
    k, t, f, start_time_str, end_time_str = d.bruteforce_dictionary(cp, pl, 'dictionary.txt', textProcess)
    entryKey.delete(0, END)
    if k != "Không tìm thấy":
        entryKey.insert(0, k.encode('utf-8').hex().zfill(16))
    else:
        entryKey.insert(0, k)
    if start_time_str:
        entryStartTime.insert(0, start_time_str)
...
def exitProgram():
    xacNhan = messagebox.askyesno("Thoát", "Bạn có chắc muốn thoát chương trình?")
    if xacNhan:
        window.destroy()
...
# Tiêu đề
labelTieuDe = Label(window, text="DES TOOL", font=("Helvetica", 18, "bold"), bg=COLOR_PRIMARY, fg="white",
                    pady=10)
...
Label(frameInput, text="Plaintext (ASCII):", font=("Helvetica", 12), bg=COLOR_BG, fg=COLOR_TEXT).grid(row=0, column=0,
                                                                                                      sticky=W, pady=5)
entryPlaintext = Entry(frameInput, font=("Helvetica", 14), width=60, bg=COLOR_ENTRY)
entryPlaintext.grid(row=0, column=1, pady=5)
Label(frameInput, text="Key (HEX, tối đa 16 kí tự):", font=("Helvetica", 12), bg=COLOR_BG, fg=COLOR_TEXT).grid(row=1,
                                                                                                               column=0,
                                                                                                               sticky=W,
                                                                                                               pady=5)
entryKey = Entry(frameInput, font=("Helvetica", 14), width=60, bg=COLOR_ENTRY)
entryKey.grid(row=1, column=1, pady=5)
Label(frameInput, text="Ciphertext (HEX):", font=("Helvetica", 12), bg=COLOR_BG, fg=COLOR_TEXT).grid(row=2, column=0,
                                                                                                     sticky=W, pady=5)
entryCiphertext = Entry(frameInput, font=("Helvetica", 14), width=60, bg=COLOR_ENTRY)
entryCiphertext.grid(row=2, column=1, pady=5)

# Màn hình xử lý
Label(window, text="Khung xử lý:", font=("Helvetica", 12), bg=COLOR_BG, fg=COLOR_TEXT).pack(anchor=W, padx=25,
                                                                                            pady=(10, 0))
...
textProcess = Text(frameProcess, width=75, height=12, font=("Courier New", 12), bd=1, relief=SOLID, bg="#FDFDFD",
...
# Thời gian bắt đầu
Label(frameStats, text="Thời gian bắt đầu:", font=("Helvetica", 12), bg=COLOR_BG, fg=COLOR_TEXT).grid(row=0, column=0,
                                                                                                      padx=20, sticky=E)
entryStartTime = Entry(frameStats, font=("Helvetica", 14), width=20, bg=COLOR_ENTRY, state='readonly')
entryStartTime.grid(row=0, column=1)
# Thời gian kết thúc
Label(frameStats, text="Thời gian kết thúc:", font=("Helvetica", 12), bg=COLOR_BG, fg=COLOR_TEXT).grid(row=0, column=2,
                                                                                                       padx=(40, 10),
                                                                                                       sticky=E)
entryEndTime = Entry(frameStats, font=("Helvetica", 14), width=20, bg=COLOR_ENTRY, state='readonly')
entryEndTime.grid(row=0, column=3)
# Thời gian chạy
Label(frameStats, text="Thời gian chạy (giây):", font=("Helvetica", 12), bg=COLOR_BG, fg=COLOR_TEXT).grid(row=1,
                                                                                                          column=0,
                                                                                                          padx=20,
                                                                                                          sticky=E)
entryTime = Entry(frameStats, font=("Helvetica", 14), width=20, bg=COLOR_ENTRY, state='readonly')
entryTime.grid(row=1, column=1)
# Số key đã thử
Label(frameStats, text="Số key đã thử:", font=("Helvetica", 12), bg=COLOR_BG, fg=COLOR_TEXT).grid(row=1, column=2,
                                                                                                  padx=(40, 10),
                                                                                                  sticky=E)
entryFrequency = Entry(frameStats, font=("Helvetica", 14), width=20, bg=COLOR_ENTRY, state='readonly')
entryFrequency.grid(row=1, column=3)

# Các nút chức năng
frameButtons = Frame(window, bg=COLOR_BG, pady=20)
frameButtons.pack()
buttonEncrypt = Button(frameButtons, text="Encrypt", font=("Helvetica", 12), width=15, command=xuLyMaHoa,
                       bg=COLOR_PRIMARY, fg="white")
buttonEncrypt.grid(row=0, column=0, padx=10)
buttonDecrypt = Button(frameButtons, text="Decrypt", font=("Helvetica", 12), width=15, command=xuLyGiaiMa,
                       bg=COLOR_PRIMARY, fg="white")
buttonDecrypt.grid(row=0, column=1, padx=10)
buttonBrute = Button(frameButtons, text="Bruteforce tuần tự", font=("Helvetica", 12), width=15, command=xuLyBruteForce,
                     bg=COLOR_PRIMARY, fg="white")
buttonBrute.grid(row=0, column=2, padx=10)
buttonDictionary = Button(frameButtons, text="Bruteforce từ điển", font=("Helvetica", 12), width=15,
                          command=xuLyBruteForceDictionary, bg=COLOR_PRIMARY, fg="white")
buttonDictionary.grid(row=0, column=3, padx=10)
buttonExit = Button(frameButtons, text="Thoát", font=("Helvetica", 12), width=15, command=exitProgram, bg=COLOR_PRIMARY,
                    fg="white")
buttonExit.grid(row=0, column=4, padx=10)
...
class DES:
    ...
    def encrypt_block(self, plaintext, key_hex):
        ...
        textProcess.insert(END, "Plaintext (hex): " + self.bin_to_hex(plaintext_bin) + "\n")
        ...
        # Thêm các bit 0 ở trước nếu khóa không đủ 16 bit hex
        key_hex = key_hex.zfill(16)
        key_bin = self.hex_to_bin(key_hex)
        textProcess.insert(END, "Key (bin): " + key_bin + "\n")
        textProcess.insert(END, "Key (hex): " + self.bin_to_hex(key_bin) + "\n")
    ...
    def decrypt_block(self, ciphertext_hex, key_hex):
        # Chuyển ciphertext từ hex sang bin
        ciphertext_bin = self.hex_to_bin(ciphertext_hex)
        textProcess.insert(END, "Ciphertext (bin): " + ciphertext_bin + "\n")
        textProcess.insert(END, "Ciphertext (hex): " + self.bin_to_hex(ciphertext_bin) + "\n")
        # Chuyển các bit thành ma trận
        ...
        # Chia C0 và D0
        C0, D0 = self.split_CD(key_pc1)
        # Tạo C, D cho các vòng
        C_matrices, D_matrices = self.generate_CD(C0, D0)
        # Vòng giải mã DES (16 vòng, subkeys đảo ngược)
        for round_num, i in enumerate(reversed(range(16))):
            textProcess.insert(END, f"------------------------------------ Round {round_num + 1} ------------------------------------\n")
            # In C và D cho vòng hiện tại
            textProcess.insert(END, "C (ma trận 4x7):\n")
            for row in C_matrices[i]:
                textProcess.insert(END, str(row) + "\n")
            textProcess.insert(END, "D (ma trận 4x7):\n")
            ...
            # Sinh subkey từ C, D
            subkey_matrix = self.permute_matrix_pc2(C_matrices[i], D_matrices[i], self.PC_2)
            textProcess.insert(END, "Subkey (ma trận 8x6):\n")
            for row in subkey_matrix:
                textProcess.insert(END, str(row) + "\n")
            # Bước mở rộng
            expanded_R = self.expand(R)
        ...
        textProcess.insert(END, "Plaintext (sau IP-1, ma trận 8x8):\n")
    ...
    def decrypt(self, ciphertext_hex, key_hex):
        ...
        for i in range(0, len(ciphertext_bytes), 8):
            ...
            textProcess.insert(END, f"\n>>> Block {i // 8 + 1}: {ciphertext_block_hex}" + "\n")
    ...
    def bruteforce(self, ciphertext_hex, plaintext_expect, textProcess=None):
        total_keys = 2 ** 64
        textProcess.insert(END, "--- Bắt đầu bruteforce tuần tự ---\n")
        textProcess.see(END)
        textProcess.update()
        ciphertext_bytes = bytes.fromhex(ciphertext_hex)
            ...
            try:
                ...
                for c_block, p_block in zip(ciphertext_blocks, plaintext_blocks):
                    ...
                    if plaintext_bytes_res != p_block:
                        success = False
                        break
                msg_try = f"Thử key (hex): {key_hex}\n"
                textProcess.insert(END, msg_try)
                textProcess.see(END)
                textProcess.update()
                ...
                if success:
                    ...
                    textProcess.insert(END, msg_found)
                    textProcess.see(END)
                    textProcess.update()
                ...
                continue
        msg_fail = "Không tìm được khóa\n"
        textProcess.insert(END, msg_fail)
        textProcess.see(END)
        textProcess.update()
    ...
    def bruteforce_dictionary(self, ciphertext_hex, plaintext_expect, dictionary_file='dictionary.txt',
                              textProcess=None):
        textProcess.insert(END, "--- Bắt đầu bruteforce từ điển ---\n")
        textProcess.see(END)
        textProcess.update()
        try:
        ...
        for key_str in keys:
            ...
            try:
                ...
                continue
        msg_fail = "Không tìm được khóa trong từ điển\n"
        textProcess.insert(END, msg_fail)
        textProcess.see(END)
        textProcess.update()
        return "Không tìm thấy", None, key_counter, None, None
